...
class InputEmbeddings(nn.Module):
    ...
    def __init(self,d_model:int,vocab_size:int):
        ...
        self.embeddigs = nn.Embedding(vocab_size,d_model)
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model:int, seq_len:int, dropout:float) ->None:
        ...
        # tensor which save not as learn parameter but need to save tensor
        self.register_buffer('pe',pe)

    def forward(self ,x):
        #required gard will make this sentence not learn because position encoding are fixed
...
class MultiheadAttentionBlock(nn.Module):
    ...
    def attention(query,key,value,mask , dropout:nn.Dropout):
        ...
        # when mask ==0 true replace by -1e9
    ...
    def forward(self,q,k,v,mask):
            #all (batch , seq_len , d_model) --> (batch , seq_len , d_model)
            query = self.w_q(q) 
            key = self.w_k(k)
            value = self.w_v(v)
...
class ResidualConnection(nn.Module):
    ...
    def forword(self, x , subplayer):
        #subplayer output of next layar (prev layer)
...
class EncoderBlock(nn.Module):
    ...
    def forward(self , x , src_mask):
        ...
        #mapping self attention (q,k,v) and x from add norm
...
class Encoder(nn.Module):
    ...
    #output from block one is input to block 2
...
class Transformer(nn.Module):
    ...
    def project(self,x):
        #map to embedding to vocab size
...
def build_transformer(src_vocab_size:int, tgt_vocab_size:int, src_seq_len:int,tgt_seq_len:int,d_model:int = 512 , N:int =6 , h:int=8,dropout:float =0.1,d_ff:int= 2048)->Transformer:
    ...
    #initialize the parameters so training will  faster not start with random values
